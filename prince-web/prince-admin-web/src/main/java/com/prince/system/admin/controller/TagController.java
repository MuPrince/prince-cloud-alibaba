...
public class TagController {
    ...
    public RespResult<PageInfo<TagDTO>> search(@RequestBody TagVo tagVo) {
        return RespResult.ok(tagService.search(tagVo));
    }
    /**
     * Description :  查询详情
     * @param id 标签id
    ...
    public RespResult<TagDTO> searchOne(@PathVariable Long id) {
        ...
        return RespResult.ok(transform.transformTo(tagService.getById(id), TagDTO.class));
    }
    /**
     * Description :  查询用户的标签
     * @param userId 用户id
    ...
    public RespResult<List<TagDTO>> getTagsByUserId(@PathVariable Long userId) {
        return RespResult.ok(tagService.getListByUserId(userId));
    }
    /**
