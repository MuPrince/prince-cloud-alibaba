package com.prince.chat.tencent.util;

import com.alibaba.fastjson.JSONObject;
import com.prince.chat.tencent.TLSSigAPIv2;
import com.prince.chat.tencent.constant.TencentCloudImConstant;
import com.prince.chat.tencent.constant.api.AccountManage;
import com.prince.chat.tencent.constant.api.SingleChatManage;
...
/**
 * Description : 腾讯IM常用方法工具类
 *
 * @author Mu Prince <br\>
 * @since 2022/6/23 16:19
 */
@Slf4j
@Component
...
public class TencentCloudImUtil {
    ...
    @Value("${tencent.cloud.im.sdkAppId}")
    private long sdkAppId = 1400696576;
    @Value("${tencent.cloud.im.key}")
    private String key = "53945c2524e01470ee2b1f24e5f47703f23fc70109414add15bf5a91a2897ebb";
    ...
    public String getTxCloudUserSig() {
        String userSig = redisTemplate.boundValueOps(REDIS_IM_USER_SIG + APP_MANAGER).get();
        if (StringUtils.isEmpty(userSig)) {
            ...
            redisTemplate.boundValueOps(REDIS_IM_USER_SIG + APP_MANAGER).set(userSig, 86400L, TimeUnit.SECONDS);
        }
        return userSig;
    }
    ...
    public void accountImport(String userId, String userName, String faceUrl) {
        ...
        String httpsUrl = getHttpsUrl(AccountManage.ACCOUNT_IMPORT, random);
    ...
    public void multiAccountImport(List<String> userIds) {
        Integer random = RandomUtils.nextInt(0, 999999999);
        String httpsUrl = getHttpsUrl(AccountManage.MULTI_ACCOUNT_IMPORT, random);
        JSONObject jsonObject = new JSONObject();
    ...
    public void accountDelete(List<String> userIds) {
        Integer random = RandomUtils.nextInt(0, 999999999);
        String httpsUrl = getHttpsUrl(AccountManage.ACCOUNT_DELETE, random);
        JSONObject jsonObject = new JSONObject();
    ...
    public String accountCheck(List<String> userIds) {
        Integer random = RandomUtils.nextInt(0, 999999999);
        String httpsUrl = getHttpsUrl(AccountManage.ACCOUNT_CHECK, random);
        JSONObject jsonObject = new JSONObject();
    ...
    public String sendMsg(Integer syncOtherMachine, String fromUserId, String toUserId, String msgType, String msgContent) {
        Integer random = RandomUtils.nextInt(0, 999999999);
        String httpsUrl = getHttpsUrl(SingleChatManage.SEND_MSG, random);
        JSONObject jsonObject = new JSONObject();
        ...
        log.info("腾讯云im单发单聊消息，请求参数：{}", jsonObject);
        String result = HttpUtil.doPost(httpsUrl, jsonObject);
    ...
    public String batchSendMsg(Integer syncOtherMachine, String fromUserId, List<String> toUserIds, String msgType, String msgContent) {
        Integer random = RandomUtils.nextInt(0, 999999999);
        String httpsUrl = getHttpsUrl(SingleChatManage.BATCH_SEND_MSG, random);
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("SyncOtherMachine", syncOtherMachine);
        if (StringUtils.isNotEmpty(fromUserId)) {
            // 发送方不为空表示指定发送用户，为空表示为管理员发送消息
        ...
        List<JSONObject> msgBody = getMsgBody(msgType, msgContent);
        jsonObject.put("MsgBody", msgBody);
        log.info("腾讯云im批量发单聊消息，请求参数：{}", jsonObject);
        String result = HttpUtil.doPost(httpsUrl, jsonObject);
    ...
    public String adminGetRoamMsg(String fromUserId, String toUserId, Integer maxCnt, Long startTime, Long endTime, String lastMsgKey) {
        Integer random = RandomUtils.nextInt(0, 999999999);
        String httpsUrl = getHttpsUrl(SingleChatManage.ADMIN_GET_ROAM_MSG, random);
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("From_Account", fromUserId);
        jsonObject.put("To_Account", toUserId);
    ...
    public void adminMsgWithDraw(String fromUserId, String toUserId, String msgKey) {
        Integer random = RandomUtils.nextInt(0, 999999999);
        String httpsUrl = getHttpsUrl(SingleChatManage.ADMIN_MSG_WITH_DRAW, random);
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("From_Account", fromUserId);
        jsonObject.put("To_Account", toUserId);
    ...
    public void adminSetMsgRead(String reportUserId, String peerUserId) {
        Integer random = RandomUtils.nextInt(0, 999999999);
        String httpsUrl = getHttpsUrl(SingleChatManage.ADMIN_SET_MSG_READ, random);
        JSONObject jsonObject = new JSONObject();
