...
import com.prince.admin.entity.vo.SystemUserLogin;
...
public class SystemUserController {
     ...
     */
    @PostMapping("login")
    public RespResult<SystemUserDTO> login(@Validated @RequestBody SystemUserLogin userLogin) {
     ...
     */
    @PutMapping
    public RespResult<?> add(@Validated({Add.class}) @RequestBody SystemUserVo vo) {
        systemUserService.addSystemUser(vo);
    ...
    public RespResult<?> del(@RequestBody List<Long> ids) {
        if (ids == null || ids.size() <= 0) {
            throw new AdminServiceException("选择删除的用户");
        }
    ...
    public RespResult<?> delById(@PathVariable Long id) {
        if (AdministratorConfig.getAdminId().equals(id)) {
            throw new AdminServiceException("系统管理员不能删除");
        }
