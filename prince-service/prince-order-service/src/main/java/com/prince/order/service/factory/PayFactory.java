package com.prince.order.service.factory;


import com.prince.order.enums.PayWayEnum;
import com.prince.order.service.PayService;
import com.prince.order.utils.SpringContextUtil;
import com.prince.order.service.payafter.PayAfterService;
...
public class PayFactory {
    ...
    // 根据类型获取支付渠道的实现类
    public PayService getPay(PayWayEnum payWayEnum) {
        Assert.notNull(payWayEnum, "付款渠道不能为空");
        return (PayService) SpringContextUtil.getBean(PAY_MAP.get(payWayEnum));
    }

    //根据类型获取结果处理的实现类
    public PayAfterService getPayAfter(PayWayEnum payWayEnum) {
        Assert.notNull(payWayEnum, "付款渠道不能为空");
        return (PayAfterService) SpringContextUtil.getBean(PAY_AFTER_MAP.get(payWayEnum));
    }
}
